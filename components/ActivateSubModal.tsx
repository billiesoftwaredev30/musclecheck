// components/ActivateSubModal.tsx
"use client";
...
import { ClientResponse, GymRatesResponse, addSubscription } from "@/lib/api";

interface ActivateSubModalProps {
...
export default function ActivateSubModal({ client, rates, onClose, onSuccess, onError, onClientsUpdated }: ActivateSubModalProps) {
  ...
  }, [startDate, duration, rates, isStudent]);
  ...
  const handleActivateSubscription = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSuccess(`Subscription activated for ${client.full_name}!`);
      onClientsUpdated();
      onClose();
    } catch (err: any) {
    ...
    >
      <div style={{ width: "100%", maxWidth: "400px", margin: "auto", animation: "modalIn 0.35s var(--ease-out-expo) both" }}>
        <GlassCard style={{ position: "relative" }}>
          ...
          <h3 style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "1.2rem", fontWeight: 700, marginBottom: "4px", color: "var(--accent-fuchsia)" }}>
            <Sparkles size={20} />
            New Subscription
          </h3>
          <p style={{ fontSize: "0.85rem", color: "var(--foreground-muted)", marginBottom: "20px" }}>
            For <strong style={{ color: "var(--foreground)" }}>{client.full_name}</strong>
          </p>

          <form onSubmit={handleActivateSubscription} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "6px", flex: 1, minWidth: "140px" }}>
                ...
                <input type="date" style={{ padding: "10px", borderRadius: "8px", border: "1px solid var(--glass-border)", background: "rgba(255,255,255,0.03)", color: "var(--foreground)", outline: "none", width: "100%" }} value={startDate} onChange={(e) => setStartDate(e.target.value)} required />
              </div>
            </div>

            <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "6px", flex: 1, minWidth: "140px" }}>
                <label style={{ fontSize: "0.85rem", fontWeight: 600, color: "var(--foreground-secondary)" }}>End Date</label>
                <input type="date" style={{ padding: "10px", borderRadius: "8px", border: "1px solid var(--glass-border)", background: "rgba(255,255,255,0.03)", color: "var(--foreground)", outline: "none", width: "100%", opacity: 0.7 }} value={endDate} disabled />
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: "6px", flex: 1, minWidth: "140px" }}>
